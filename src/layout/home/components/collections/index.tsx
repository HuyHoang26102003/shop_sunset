...
export function Collections(props: CollectionsProps) {
    ...
    () =>
      data?.map((value, index) => (
        <div key={index} className="keen-slider__slide number-slide1">
          <Styles.Content>
            <Typography as="h3" fontWeight="500" size="lg" color="heading">
              {value.title}
            </Typography>
            <Button as="a" href={value.action.path}>
              {value.action.label}
            </Button>
          </Styles.Content>
          <Styles.Figure>
  ...
  const renderDots = useMemo(
    () =>
      ...
      [
        ...Array(
          (instanceRef?.current?.track.details.slides.length || 2) - 1 || 0
        ).fill(2),
      ].map((_, index) => (
        <Styles.Dot
          key={index}
  ...
  return (
    <Container size="lg">
      <Styles.Container>
        <SectionTitle>Bộ sưu tập</SectionTitle>
